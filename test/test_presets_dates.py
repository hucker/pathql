...
@pytest.fixture
def edge_case_file(tmp_path: pathlib.Path) -> typing.Generator[pathlib.Path, None, None]:
...
def make_mock_stat(modified_time: float) -> os.stat_result:
    """Return a mock stat_result object with creation time set to modified_time."""
    # Create a real stat_result tuple, but override times
    stat = list(os.stat_result((0,) * 10))
    stat[7] = modified_time  # st_size
    stat[8] = modified_time  # st_atime
    stat[9] = modified_time  # st_mtime
    # st_ctime is not always at the same index, but for most platforms it's 10
    if len(stat) > 10:
        stat[10] = modified_time  # st_ctime
